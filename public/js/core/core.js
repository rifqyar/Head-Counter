...
function onAjaxError(err, statusCode = null) {
  ...
  Toastify({
    text: err.responseJSON?.message == undefined ? "Something went error!, Please try again" : err.responseJSON.message,
    duration: 3000,
    close:true,
    gravity:"top",
    position: "right",
...
function fillResData(form_id){
  ...
  for (let i = 0; i < form.length; i++) {
      ...
      if ($(form[i]).attr('multiple') != 'multiple'){
        if($(form[i]).attr('type') == 'checkbox' || $(form[i]).attr('type') == 'radio'){
...
function apiCall(url, method, form_id, header = null, beforeAjax = null, onError = null, showError = true, callback) {
